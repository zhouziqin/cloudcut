package com.mainiway.cloudcut.redis.service.impl;
...
@Service
public class DubboRedisServcieImpl implements ISRedisService {
	...
	@Override
	public <T> T get(final String key,final Class clz) {
		try {
			return (T) redisTemplate.execute(new RedisCallback<Object>() {
				@Override
				public Object doInRedis(RedisConnection connection) throws DataAccessException {
					...
					if (null == val || 0 == val.length) {
 					...
 					T result =  (T) JSON.parseObject(new String(val), clz);
					return result;
				}
			}) ;
		} catch (Exception e) {
